...
class Antinuke(commands.Cog):
    ...
    async def on_member_ban(self, guild, member):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
             ...
             await guild.unban(user=member, reason="Cristal Security | Auto Recovery")
             await logs.user.ban(reason=f"Cristal Security | Anti Ban")
    ...
    async def on_guild_emojis_update(self, emoji):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"Cristal Security | Anti Emoji Update")
    ...
    async def on_guild_emoji_create(self, emoji):
            ...
            logs = logs[0]
            await emoji.delete()
            await logs.user.ban(reason=f"Cristal Security | Anti Emoji Create")
    ...
    async def on_guild_emojis_delete(self, emoji):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"Cristal Security | Anti Emoji Delete")
    ...
    async def on_sticker_create(self, sticker):
            ...
            await logs.user.ban(reason=f" Cristal Security | Anti Sticker Create")
    ...
    async def on_sticker_update(self, sticker):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"Cristal Security | Anti Update Create")
    ...
    async def on_sticker_delete(self, sticker):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"Cristal Security | Anti Sticker Update")      
    ...
    async def on_member_remove(self, member, guild):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
             ...
             await logs.user.ban(reason=f"Cristal Security Security | Anti Kick")
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
             ...
             await logs.user.ban(reason=f"Cristal Security Security | Anti Prune")
    ...
    async def on_guild_channel_delete(self, channel):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await channel.clone(reason="Cristal Security | Auto Recovery")
              await logs.user.ban(reason=f"Cristal Security | Anti Channel Delete")
    ...
    async def on_guild_channel_update(self, before, after):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await after.edit(name=f"{before.name}", nsfw=f"{before.nsfw}", reason="Cristal Security | Auto Recovery")
              await logs.user.ban(reason=f"Cristal Security | Anti Channel Update")
    ...
    async def on_guild_role_create(self, role):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await role.delete(reason="Cristal Security | Auto Recovery")
              await logs.user.ban(reason=f"Cristal Security | Anti Role Create")
    ...
    async def on_guild_role_delete(self, role):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason=f"Cristal Security | Anti Role Delete")
    ...
    async def on_guild_role_update(self, before, after):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await after.edit(name=f"{before.name}",  hoist=f"{before.hoist}", mentionable=f"{before.mentionable}", reason="Cristal Security | Auto Recovery")
              await logs.user.ban(reason=f"Cristl Security | Anti Role Update")
    ...
    async def on_guild_update(self, before, after):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason="Cristal Security | Anti Guild Update")
    ...
    async def on_member_unban(self, guild, member):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 877922339382243328 or logs.user.id == 918527734748164128 or str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason=f"Cristal Security | Anti Unban")
    ...
    async def on_webhooks_update(self, webhook):
      with open('whitelisted.json') as f:
            ...
            if str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason=f"Cristal Security | Anti Webhook Update")
    ...
    async def on_webhooks_create(self, channel):
      with open('whitelisted.json') as f:
            ...
            if str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason=f"Cristal | Anti Webhook Create")
    ...
    async def on_webhooks_delete(self, webhook):
      with open('whitelisted.json') as f:
            ...
            if str(logs.user.id) in whitelisted[str(guild.id)]:
              ...
              await logs.user.ban(reason=f"Cristal Security | Anti Webhook Delete")
    ...
    async def on_member_join(self, member):
      with open('whitelisted.json') as f:
        ...
        if logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
         ...
         if member.bot:
          await member.ban(reason="Cristal Security | Anti Bot Add")
          await logs.user.ban(reason=f"Cristal Security | Anti Bot Add")
